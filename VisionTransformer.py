...
class LinearProjectionOfFlattendPatches(nn.Module):
  def __init__(self, dim:int, MLP_dim:int, patchWidth:int, patchHigh:int, c:int, w:int, h:int):
    super(LinearProjectionOfFlattendPatches, self).__init__()
    self.patchWidth = patchWidth
    self.patchHigh = patchHigh
    
    patchDim = self.patchHigh * self.patchWidth * c
    self.clsToken = nn.Parameter(torch.randn(1, patchDim))
    assert h % patchHigh == 0 and w % patchWidth == 0, 'h and w must be divided by ph and pw'
    self.positionEmbding = nn.Linear(patchDim, dim)
...
class TransformerEncoder(nn.Module):
  def __init__(self, L:int, dim:int, MLP_dim:int, k:int, patchWidth:int, patchHigh:int, c:int, w:int, h:int):
...
class VisionTransformer(nn.Module):
  def __init__(self, L: int, dim:int, MLP_dim:int, k: int, clsNum: int, c: int, w: int, h: int, patchWidth: int, patchHigh: int):
    '''
    ...
    self.linearProjectionOfFlattendPatches = LinearProjectionOfFlattendPatches(dim, MLP_dim, patchWidth, patchHigh, c, w, h)
    self.transformerEncoder = TransformerEncoder(L, dim, MLP_dim, k, patchWidth, patchHigh, c, w, h)
...
if __name__ == '__main__':
  ...
  net = VisionTransformer(1, 768, 3072, 1, 10, 3, 224, 224, 16, 16)
  y = net(image)
  print(net, y.shape)
  # image = torch.randn(3, 3, 224, 224)
